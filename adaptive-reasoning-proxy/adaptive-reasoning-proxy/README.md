...
A drop-in FastAPI proxy for any OpenAI-compatible API. It intercepts the token
...
client ──> proxy (:8000) ──> llama-server / any OpenAI-compatible API (:8080)
                ...
                ├─ task router (regex or embeddings)
...
`EMBEDDINGS_ENABLED`, `BASELINE_TOKENS`, `COST_PER_1K_TOKENS`.
...
- Embedding classifier and DTW profile matching are optional enrichment layers
  that degrade gracefully — the proxy works with neither enabled.
